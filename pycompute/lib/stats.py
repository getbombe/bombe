...
class Stats:
	...
	def poly_regression (data):
		...
		x_itp = list(linspace(x_dat[0], x_dat[len(x_dat)-1], len(x_dat)))

		coeffs = polyfit(x_dat, y_dat, order)
		polynom = poly1d (coeffs)

		if data['removal'] == 'subtract':
			data['data']['y'] = list(subtract(y_dat, list(polynom(x_itp)))) 
		elif data['removal'] == 'divide':
			data['data']['y'] = list(divide(y_dat, list(polynom(x_itp))))
		else:
